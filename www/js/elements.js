...
const completeNode = (id, ip /* internal IP*/, containers) => {
    ...
    const actions = nodeHTML ? nodeHTML.querySelectorAll('.action') : [];
...
const container = (container, _MemTotal) => {
    ...
    const used_memory = memory_stats.usage - (((_a = memory_stats === null || memory_stats === void 0 ? void 0 : memory_stats.stats) === null || _a === void 0 ? void 0 : _a.cache) || 0);
    ...
    if (deletedContainers.indexOf(id) > 0)
        return 'DELETED';
    // add colors to services
    const colors = ['blue', 'yellow', 'red', 'green', 'orange', 'violet'];
    let service = (_b = labels['com.docker.swarm.service.name']) !== null && _b !== void 0 ? _b : '';
    if (service && !services.includes(service))
        services.push(service);
    const color = service && colors[services.indexOf(service) % colors.length];
    const name = labels['com.docker.swarm.service.name'] || `${image} / ${names.map(n => n.replace(/^\//gm, '')).join(', ')}`;
