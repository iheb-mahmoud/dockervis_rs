...
export const calculateCPUUsage = stats => {
    ...
    let cpuPercent = 0.0;
    try {
        if (!stats.precpu_stats.cpu_usage.total_usage)
            throw new Error('precpu_stats.cpu_usage.total_usage is missing');
        if (!stats.precpu_stats.system_cpu_usage)
            throw new Error('stats.precpu_stats.system_cpu_usage is missing');
